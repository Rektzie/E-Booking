...
class Booking_student(models.Model):
    ...
    teacher_user_id = models.ForeignKey(Teacher, on_delete=models.CASCADE)
    staff_user_id = models.ForeignKey(Staff, on_delete=models.CASCADE)
    teacher_date = models.DateTimeField(default=datetime.now, blank=True)
    teacher_result = models.BooleanField(default=False)
    staff_date = models.DateTimeField(default=datetime.now, blank=True)
